...
describe("Timeout", () => {
  ...
  it("Make sure the Upload image - contains invalid images can be saved after clicking on extend button", () => 
  {
    loginActions.visitPage();
    loginActions.inputUserName(user.username);
    loginActions.inputPassword(user.password);
  ...
  it("Make sure lesion 1 - completed can be saved", () => 
  {
    loginActions.visitPage();
    loginActions.inputUserName(user.username);
    loginActions.inputPassword(user.password);
    ...
    dashboardActions.assertElement('#f_site');
  });
});